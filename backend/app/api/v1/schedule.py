...
async def bulk_create_schedules(
    ...
    """Bulk create schedules (Manager only)"""
    if current_user.role not in ['manager', 'admin']:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only managers can create schedules"
    ...
    try:
        ...
        if not schedules or not isinstance(schedules, list):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid schedules data. Expected list of schedules."
            )
        ...
        for schedule_data in schedules:
            try:
                ...
                schedule = service.create_schedule(schedule_create, current_user.id)
                created_schedules.append(schedule)
            except Exception as e:
                logger.error(f"Error creating schedule in bulk: {e}")
                # Continue with other schedules
        
        # Send real-time notification
        if created_schedules:
            await websocket_manager.notify_schedule_updated(
                schedule_data={"type": "bulk_update"},
                department_id=current_user.department_id
            )
        ...
        logger.error(f"Error bulk creating schedules: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error creating schedules"
        )
...
@router.get("/staff-statistics/{staff_id}")
async def get_staff_statistics(
    ...
    try:
        service = ScheduleService(db)
        stats = service.get_staff_statistics(staff_id, start_date, end_date)
        return stats
    except Exception as e:
        logger.error(f"Error getting staff statistics: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error retrieving staff statistics"
        )
...
async def get_department_statistics(
    ...
    """Get aggregated statistics for all staff in manager's department"""
    if current_user.role not in ['manager', 'admin']:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only managers can view department statistics"
        )
